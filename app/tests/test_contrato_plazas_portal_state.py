...
sys.modules.setdefault(
    "supabase",
    SimpleNamespace(create_client=lambda *args, **kwargs: None, Client=object),
)
sys.modules.setdefault("fpdf", SimpleNamespace(FPDF=object))
...
class _DummyPortalPlazasNavState:
    ...
    def __init__(self):
        ...
        self.ruta_contrato_principal = "/portal/contratos"
...
class _DummyContratoPlazasState:
    ...
    _fila_configuracion = ContratoPlazasState._fila_configuracion
    ...
    @property
    def categorias_resumen_simple(self):
        return self._computed("categorias_resumen_simple")


def test_ruta_plazas_principal_usa_primer_contrato_con_personal():
    dummy = _DummyPortalPlazasNavState()

    assert dummy.ruta_plazas_principal.fget(dummy) == "/portal/contratos"
...
def test_filas_tabla_muestran_sueldo_sede_y_cta_contextual():
    ...
    assert filas[1]["cta_texto"] == "Ir a empleados"
    assert filas[2]["tiene_sede"] is False
    assert filas[2]["sede_display_tabla"] == ""
    assert filas[2]["cta_texto"] == "Asignar sede"


def test_resumen_categorias_operativo_incluye_min_max_y_pendientes():
    dummy = _DummyContratoPlazasState()

    categorias = dummy.categorias_resumen_simple

    assert len(categorias) == 2
    assert categorias[0]["nombre"] == "Jardinero B"
    assert categorias[0]["minimo_texto"] == "1"
    assert categorias[0]["maximo_texto"] == "2"
    assert categorias[0]["configuradas_texto"] == "2"
    assert categorias[0]["ocupadas_texto"] == "1"
    assert categorias[0]["pendientes_texto"] == "0"
    assert categorias[0]["estado"] == "COMPLETA"
    assert categorias[1]["nombre"] == "Podador"
    assert categorias[1]["configuradas_texto"] == "0"
    assert categorias[1]["ocupadas_texto"] == "0"
    assert categorias[1]["pendientes_texto"] == "1"
    assert categorias[1]["estado"] == "DEBAJO_MINIMO"
