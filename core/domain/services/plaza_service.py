...
try:
    ...
    services_pkg = sys.modules.get("core.domain.services")
...
class PlazaService:
    ...
    async def empleado_tiene_plaza_activa_con_categoria(
        ...
        *,
        ...
        return await self.repository.empleado_tiene_plaza_activa_con_categoria(
            empleado_id,
            excluir_plaza_id=excluir_plaza_id,
        )

    async def _sincronizar_historial_asignacion(
        self,
        empleado_id: int,
        plaza_id: int,
    ) -> None:
        """Registra en historial la plaza activa del empleado."""
        from core.domain.services import historial_laboral_service

        registro_activo = await historial_laboral_service.obtener_registro_activo(
            empleado_id,
        )
        plaza_activa_id = int(getattr(registro_activo, "plaza_id", 0) or 0)

        if plaza_activa_id == plaza_id:
            return
        if plaza_activa_id > 0:
    ...
    async def _sincronizar_historial_liberacion(
        ...
        """Libera en historial la plaza cuando sigue marcada como activa."""
        from core.domain.services import historial_laboral_service

        registro_activo = await historial_laboral_service.obtener_registro_activo(
            empleado_id,
        )
        if int(getattr(registro_activo, "plaza_id", 0) or 0) != plaza_id:
            return

        await historial_laboral_service.liberar_plaza_empleado(empleado_id)
    ...
    async def sincronizar_categorias_desde_contrato(self, contrato_id: int) -> int:
        ...
        """
        from core.domain.services import contrato_categoria_service
    ...
    async def actualizar(self, id: int, plaza_update: PlazaUpdate) -> Plaza:
        ...
        empleado_original_id = plaza.empleado_id
        sede_original_id = plaza.sede_id
        salario_original = Decimal(str(plaza.salario_mensual or 0))
        ...
        if (
            empleado_original_id is not None
            and "sede_id" in cambios
            and plaza.sede_id != sede_original_id
        ):
            raise BusinessRuleError(
        ...
        salario_actualizado = Decimal(str(plaza_actualizada.salario_mensual or 0))
        ...
        if (
            ...
            await historial_laboral_service.registrar_cambio_salario(
                empleado_id=int(plaza_actualizada.empleado_id or 0),
                plaza_id=int(plaza_actualizada.id or id),
                salario_anterior=salario_original,
                salario_nuevo=salario_actualizado,
            )
    ...
    async def liberar_plaza(self, plaza_id: int) -> Plaza:
        ...
        if empleado_id > 0:
            ...
            await self._sincronizar_historial_liberacion(empleado_id, plaza_id)
            tiene_plaza_activa = await self.empleado_tiene_plaza_activa_con_categoria(
                empleado_id,
    ...
    async def _validar_sede_activa(self, sede_id: int) -> None:
        from core.domain.services import sede_service
    ...
    def _map_resumen(self, item: dict) -> PlazaResumen:
        return PlazaResumen(
            ...
            empleado_uuid=item.get("empleado_uuid", ""),
