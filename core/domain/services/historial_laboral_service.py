...
- Se cambia el sueldo en una plaza ocupada (CAMBIO_SALARIO)
...
from core.domain.repositories import SupabaseHistorialLaboralRepository
from core.core.exceptions import NotFoundError, DatabaseError
from core.domain.models.historial_laboral import (
    HistorialLaboral,
    HistorialLaboralInterno,
    HistorialLaboralResumen,
)
from core.core.enums import TipoMovimiento, EstatusPlaza
...
class HistorialLaboralService:
    ...
    async def registrar_cambio_salario(
        ...
        *,
        ...
        """Registra un cambio de sueldo manteniendo la plaza ocupada."""
        fecha_movimiento = fecha or date.today()
        ...
        nota_final = notas or (
            "Cambio de sueldo "
            f"de ${salario_anterior:,.2f} a ${salario_nuevo:,.2f}"
        )
        datos = HistorialLaboralInterno(
            empleado_id=empleado_id,
            plaza_id=plaza_id,
            tipo_movimiento=TipoMovimiento.CAMBIO_SALARIO,
            fecha_inicio=fecha_movimiento,
            notas=nota_final,
        )
        ...
        logger.info(
            "Registrado CAMBIO_SALARIO de empleado %s en plaza %s: %s -> %s",
            empleado_id,
            plaza_id,
            salario_anterior,
            salario_nuevo,
