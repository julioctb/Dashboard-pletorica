...
from core.core.text_utils import formatear_moneda, normalizar_mayusculas
from core.core.utils import generar_candidatos_codigo
from core.domain.models.categoria_puesto import (
    CategoriaPuestoCreate,
    CategoriaPuestoUpdate,
)
from core.domain.services import categoria_puesto_service, tipo_servicio_service
from core.core.exceptions import NotFoundError, DuplicateError, DatabaseError, BusinessRuleError
...
class CategoriasPuestoState(BaseState):
    ...
    error_orden: str = ""
    error_salario_base_mensual: str = ""
    ...
    def set_form_salario_base_mensual(self, value: str):
        self.form_salario_base_mensual = formatear_moneda(value) if value else ""
        self.error_salario_base_mensual = ""
    ...
    def validar_salario_base_mensual_campo(self):
        self.error_salario_base_mensual = ""
        if not self.form_salario_base_mensual:
            return
        try:
            salario = self._parse_salario_base_mensual()
        except ValueError as e:
            self.error_salario_base_mensual = str(e)
            return

        if salario is not None and salario < 0:
            self.error_salario_base_mensual = "El sueldo base no puede ser negativo"
    ...
    def validar_todos_los_campos(self):
        ...
        self.validar_salario_base_mensual_campo()

    def _parse_salario_base_mensual(self) -> Optional[Decimal]:
        if not self.form_salario_base_mensual or not self.form_salario_base_mensual.strip():
            return None
        limpio = (
            self.form_salario_base_mensual
            .replace(",", "")
            .replace("$", "")
            .replace(" ", "")
            .strip()
        ...
        try:
            ...
            raise ValueError("Capture un sueldo base valido") from e

    @staticmethod
    def _serializar_categoria_state(categoria) -> dict:
        data = categoria.model_dump(mode="json") if hasattr(categoria, "model_dump") else dict(categoria)
        salario = data.get("salario_base_mensual")
        data["salario_base_mensual_fmt"] = (
            formatear_moneda(str(salario))
            if salario not in (None, "")
            else "-"
        )
        return data
    ...
    async def _fetch_categorias(self):
        ...
        try:
            ...
            self.categorias = self.serializar_lista_state(
                categorias,
                transformar=self._serializar_categoria_state,
            )
    ...
    def abrir_modal_editar(self, categoria: dict):
        ...
        self.form_salario_base_mensual = formatear_moneda(str(salario)) if salario not in (None, "") else ""
    ...
    async def _crear_categoria(self):
        categoria_create = CategoriaPuestoCreate(
            ...
            salario_base_mensual=self._parse_salario_base_mensual(),
        )
