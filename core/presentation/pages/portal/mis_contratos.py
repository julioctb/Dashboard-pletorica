...
from core.presentation.pages.backoffice.contratos.contratos_modals import modal_contrato
from core.presentation.pages.backoffice.contratos.contratos_state import ContratosState
...
from core.presentation.theme import Colors, Typography, Spacing
from core.domain.services import contrato_service, contrato_categoria_service
from core.core.exceptions import DatabaseError, BusinessRuleError
...
from core.presentation.pages.backoffice.contratos.contrato_detail_sections import (
...
class MisContratosState(PortalState):
    ...
    @rx.event
    def ir_a_plazas_contrato(self, contrato_id: int | str):
    ...
    def ir_a_plazas_contrato_detalle(self):
        contrato_id = int(self.contrato_detalle.get("id") or 0)
        if contrato_id <= 0:
            return rx.redirect("/portal/contratos")
        self.cerrar_detalle()
        return rx.redirect(f"/portal/contratos/{contrato_id}/plazas")
...
def _card_contrato(cto: dict) -> rx.Component:
    ...
    return entity_card(
        ...
        acciones=rx.cond(
            MisContratosState.puede_navegar_plazas_desde_contratos & cto["tiene_personal"],
            rx.button(
                ...
                variant="soft",
                color_scheme=Colors.PORTAL_ACCENT_SCHEME,
                on_click=MisContratosState.ir_a_plazas_contrato(cto["id"]).stop_propagation,
            ),
            rx.fragment(),
        ),
